...
class AuthWindow(ctk.CTkToplevel):
    def __init__(self, master):
        ...
        # Definimos las fuentes que usarán los widgets
        self.main_font = ("Arial", 14)
        self.bold_font = ("Arial", 14, "bold")
        self.title_font = ("Arial", 28, "bold")

        # Llamamos a las funciones que construyen la interfaz
        self._load_images()
        self._setup_ui()

    def _load_images(self):
            # Construye la ruta a la carpeta de imágenes de forma segura
            images_path = Path(__file__).parent / "Imagenes"

            # --- CÓDIGO DE DEPURACIÓN PARA VER QUÉ ESTÁ PASANDO ---
            print("--- Depurando Rutas de Imágenes ---")
            print(f"La carpeta del script es: {Path(__file__).parent}")
            print(f"Buscando la carpeta de imágenes en: {images_path}")
            
            # Verificamos la primera imagen: adorno.png
            ruta_adorno = images_path / "adorno.png"
            print(f"Ruta completa para adorno.png: {ruta_adorno}")
            print(f"¿Existe el archivo adorno.png? -> {ruta_adorno.exists()}")

            # Verificamos la segunda imagen: fondo.png
            ruta_fondo = images_path / "fondo.png"
            print(f"Ruta completa para fondo.png: {ruta_fondo}")
            print(f"¿Existe el archivo fondo.png? -> {ruta_fondo.exists()}")
            print("------------------------------------")
            # --- FIN DEL CÓDIGO DE DEPURACIÓN ---

            # Carga las imágenes usando la ruta completa
            self.login_logo = self._load_image_with_retry(ruta_adorno, (150, 150), mode="fit")
            self.background_image = self._load_image_with_retry(ruta_fondo, (400, 600), mode="cover")
    ...
    def handle_login(self):
        ...
        if user_data and password == user_data[3]:
            ...
            # Abrimos la app del blog como una nueva ventana Toplevel
