# logindiseño.py (AJUSTE FINAL Y CORREGIDO)
...
class AuthWindow(ctk.CTkToplevel):
    ...
    def handle_login(self):
        ...
        # Comparamos la contraseña con el segundo elemento (índice 1) de la tupla.
        if user_data and password == user_data[1]:
            db_user_id = user_data[0]
            
            # Liberamos el foco de la ventana modal
            self.grab_release()
            # Destruimos la ventana de login actual
            self.destroy()
            
            # Creamos la ventana principal del blog.
    ...
    def _load_images(self):
        try:
            ...
            messagebox.showerror("Error de Imagen", f"No se pudieron cargar las imágenes: {e}")
