...
import oracledb 
from articulosCategorias_diseño import BlogApp
...
def handle_login():
    ...
    if not email or not password:
        messagebox.showerror("Error", "Por favor, introduce tu correo electrónico y contraseña.")
        return
        
    user_data = user_exists(email)
    
    if user_data:
        # user_data es una tupla: (user_id, username, email, password)
        db_user_id = user_data[0]
        db_username = user_data[1]
        db_stored_password = user_data[3]

        if password == db_stored_password: 
            # 1. Oculta la ventana de login antes de mostrar el mensaje
            app.withdraw() 
            messagebox.showinfo("Éxito", f"¡Bienvenido, {db_username}! Iniciando la aplicación.")
            
            # 2. Cierra DEFINITIVAMENTE la ventana de login
            app.destroy()

            # 3. Inicia la ventana principal del blog pasándole el ID del usuario
            blog_app = BlogApp(user_id=db_user_id)
            blog_app.mainloop()
            return # Termina la función aquí

    # Si la función llega hasta acá, es porque el login falló
